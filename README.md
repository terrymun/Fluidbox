...
- **Binding Fluidbox to hidden images.** As Fluidbox requires access to the final calculation dimensions of the image in question in order to (1) position the ghost element correctly and (2) calculate the correct scale factor and transform values, it will break if you try to bind it to hidden elements that are not visible. Instead, try to rebind Fluidbox to revealed elements. This also applies to dynamically-loaded content, see demo for a working example.
